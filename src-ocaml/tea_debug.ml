...
let debug:
  ...
  =
  let client_msg msg = ((ClientMsg (msg))[@explicit_arity ]) in
  fun string_of_msg  ->
    fun { init; update; view; subscriptions; shutdown }  ->
      let init' (flags : 'flags) =
        ...
        ({
           ...
           state = Running;
           show_details = false
         }, (cmd |> (Tea_cmd.map client_msg))) in
      let update' model =
        function
        | ((ClientMsg (msg))[@explicit_arity ]) ->
            ...
            then
              ...
              let dmodel' =
                {
                  model with
                  history = (((string_of_msg msg), cmodel') ::
                    (model.history))
                } in
        ...
        | TogglePaused  ->
            (match model.state with
             ...
             | Running  ->
                 ({ model with state = ((Paused (0))[@explicit_arity ]) },
                   Tea_cmd.none))
        | ((SelectHistoryItem (i))[@explicit_arity ]) ->
            ({ model with state = ((Paused (i))[@explicit_arity ]) },
              Tea_cmd.none)
        | ToggleDetails  ->
            ({ model with show_details = (not model.show_details) },
              Tea_cmd.none) in
      let view_styles () =
        let open Tea_html2 in
          let rule selector properties =
            (((properties |> (List.map (fun (k,v)  -> k ^ (":" ^ v)))) |>
                (String.concat ";"))
      ...
      |}] in
            aside [A.class' "details"] [(model |> format) |> text] in
          ...
          let module A = Tea_html2.Attributes in
            let module E = Tea_html2.Events in
              let (selected_index,selected_model,paused) =
                ...
                | ((Paused (index))[@explicit_arity ]) ->
                    (index, ((List.nth model.history index) |> snd), true) in
                ...
                div [A.id "debug"; A.classList [("paused", paused)]]
                  ...
                  nav []
                    [div
                       ...
                       if paused
                       then A.title "click to resume"
                       else A.title "click to pause"]
                       [(Printf.sprintf "Explore History (%d)" history_count)
                          |> text];
                    if paused
                    then view_history model selected_index
                    else noNode]]] in
      let subscriptions' model =
        (((model.history |> List.hd) |> snd) |> subscriptions) |>
          (Tea_sub.map client_msg) in
      let shutdown' model =
        (((model.history |> List.hd) |> snd) |> shutdown) |>
          (Tea_cmd.map client_msg) in
      {
        init = init';
        update = update';
        view = view';
  ...
  ('model,'msg) Tea_app.beginnerProgram ->
    ('msg -> string) ->
      Web.Node.t Js.null_undefined ->
        unit -> 'msg debug_msg Tea_app.programInterface
  =
            ...
            debug string_of_msg
              {
                ...
                update =
                  (fun model  ->
                     fun msg  -> ((update model msg), Tea_cmd.none));
                view;
...
let standardProgram:
  ('flags,'model,'msg) Tea_app.standardProgram ->
    ('msg -> string) ->
      Web.Node.t Js.null_undefined ->
        'flags -> 'msg debug_msg Tea_app.programInterface
  =
    ...
    fun string_of_msg  ->
      fun pnode  ->
        fun flags  ->
          let debugged =
            debug string_of_msg
              { init; update; view; subscriptions; shutdown } in
          Tea_app.program debugged pnode flags