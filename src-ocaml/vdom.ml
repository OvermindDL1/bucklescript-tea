...
let noNode: 'msg t = ((CommentNode (""))[@explicit_arity ])
...
let prop (key : string) (value : string) =
  (((RawProp (key, value))[@implicit_arity ]) : 'msg property)
let onCB (name : string) (key : string) (cb : Web.Node.event -> 'msg option)
  =
  (((Event
       (name, ((EventHandlerCallback (key, cb))[@implicit_arity ]),
         (ref None)))[@implicit_arity ]) : 'msg property)
...
let data (key : string) (value : string) =
  (((Data (key, value))[@implicit_arity ]) : 'msg property)
let style (key : string) (value : string) =
  (((Style ([(key, value)]))[@explicit_arity ]) : 'msg property)
...
let rec renderToHtmlString: 'msg t -> string =
  ...
  | ((Node (namespace,tagName,_key,_unique,props,vdoms))[@implicit_arity ])
      ->
      let renderProp =
        function
        | NoProp  -> ""
...
let emptyEventHandler: Web.Node.event_cb = ((fun _ev  -> ())[@bs ])
...
let rec patchVNodesOnElems_PropertiesApply
  ...
  (((match (oldProperties, newProperties) with
     ...
     | ((((Event (oldName,oldHandlerType,oldCache))[@implicit_arity ]) as
           _oldProp)::oldRest,(((Event
                                 (newName,newHandlerType,newCache))[@implicit_arity
                                                                    ])
                                 as _newProp)::newRest)
         ->
         let () =
         ...
         patchVNodesOnElems_PropertiesApply callbacks elem (idx + 1) oldRest
           newRest
     | ((((Style (oldS))[@explicit_arity ]) as oldProp)::oldRest,(((Style
                                                                    (newS))
                                                                    [@explicit_arity
                                                                    ]) as
                                                                    newProp)::newRest)
...
and patchVNodesOnElems_MutateNode (callbacks : 'msg applicationCallbacks ref)
  ...
  (match (oldNode, newNode) with
   | ((((Node
         (_oldNamespace,oldTagName,_oldKey,oldUnique,oldProperties,oldChildren))
         [@implicit_arity ]) as _oldNode),(((Node
                                             (_newNamespace,newTagName,_newKey,newUnique,newProperties,newChildren))
                                             [@implicit_arity ]) as newNode))
       ->
  ...
  (((match (oldVNodes, newVNodes) with
     ...
     | (((Text (oldText))[@explicit_arity ])::oldRest,((Text
        (newText))[@explicit_arity ])::newRest) ->
         ...
         patchVNodesOnElems callbacks elem elems (idx + 1) oldRest newRest
     | (((LazyGen
        (oldKey,_oldGen,oldCache))[@implicit_arity ])::oldRest,((LazyGen
        (newKey,newGen,newCache))[@implicit_arity ])::newRest) ->
         if oldKey = newKey
         then
           let () = newCache := (!oldCache) in
           patchVNodesOnElems callbacks elem elems (idx + 1) oldRest newRest
           ...
           (match (oldRest, newRest) with
            | (((LazyGen
               (olderKey,_olderGen,_olderCache))[@implicit_arity ])::olderRest,((LazyGen
               (newerKey,_newerGen,_newerCache))[@implicit_arity ])::newerRest)
                ...
                patchVNodesOnElems callbacks elem elems (idx + 2) olderRest
                  newerRest
            | (((LazyGen
               (olderKey,_olderGen,olderCache))[@implicit_arity ])::olderRest,_)
     ...
     | ((((Node
           (oldNamespace,oldTagName,oldKey,_oldUnique,_oldProperties,_oldChildren))
           [@implicit_arity ]) as oldNode)::oldRest,(((Node
                                                       (newNamespace,newTagName,newKey,_newUnique,_newProperties,_newChildren))
                                                       [@implicit_arity ]) as
                                                       newNode)::newRest)
         ->
         ...
         then
           ...
           then
             ...
             (match (oldRest, newRest) with
              | (((Node
                 (olderNamespace,olderTagName,olderKey,_olderUnique,_olderProperties,_olderChildren))
                 [@implicit_arity ])::olderRest,((Node
                 (newerNamespace,newerTagName,newerKey,_newerUnique,_newerProperties,_newerChildren))
                 [@implicit_arity ])::newerRest) when
              ...
              | (((Node
                 (olderNamespace,olderTagName,olderKey,_olderUnique,_olderProperties,_olderChildren))
                 [@implicit_arity ])::olderRest,_) when
                  (olderNamespace = newNamespace) &&
              ...
              | (_,((Node
                 (newerNamespace,newerTagName,newerKey,_newerUnique,_newerProperties,_newerChildren))
                 [@implicit_arity ])::_newerRest) when
                  (oldNamespace = newerNamespace) &&
...
let wrapCallbacks_On :
  'a 'b . ('a -> 'b) -> 'a systemMessage -> 'b systemMessage= fun (type a) ->
  fun (type b) ->
  (fun func  ->
     function
     | Render  -> Render
     ...
     | ((RemoveRenderMsg (msg))[@explicit_arity ]) ->
         ((RemoveRenderMsg ((func msg)))[@explicit_arity ]) : (a -> b) ->
                                                                a
                                                                  systemMessage
                                                                  ->
                                                                  b
       ...
       ref
         {
           ...
           on =
             (fun smsg  ->
               ...
               b applicationCallbacks ref -> a applicationCallbacks ref)
let map: ('a -> 'b) -> 'a t -> 'b t =
  fun func  ->
    fun vdom  ->
