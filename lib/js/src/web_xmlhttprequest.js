...
var Web_formdata            = require("./web_formdata");
...
function getResponseHeader(key, x) {
  return Js_primitive.js_from_nullable(x.getResponse(key));
}
...
function readyState(x) {
  var i = x.readystate;
  if (i > 4 || i < 0) {
    var s = "Invalid return from 'readystate' of: " + i;
    throw [
          Caml_builtin_exceptions.failure,
          s
        ];
...
function get_responseXML(x) {
  return Js_primitive.js_from_nullable(x.responseXML);
}
