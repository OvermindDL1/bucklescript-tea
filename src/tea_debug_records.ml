...
let wrap :
  ('flags, 'model, 'msg) options ->
  ('flags, 'model debug_model, 'msg debug_msg) Tea_app.standardProgram
  =
  let client_msg msg = ClientMsg msg in
  fun { init; update; view; subscriptions; string_of_msg; } ->
...
let standardProgram :
  ...
  =
  fun options pnode flags ->
    Tea_app.standardProgram (wrap options) pnode flags
