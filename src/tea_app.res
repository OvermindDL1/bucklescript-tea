...
let programLoop = (update, view, subscriptions, initModel, initCmd, x) =>
  ...
  | Some(parentNode) =>
    callbacks => {
      ...
      let noFrameID = Some(Obj.magic(-1))
      let nextFrameID: ref<option<Webapi.rafId>> = ref(None)
      let doRender = _delta =>
      ...
      let scheduleRender = () =>
        ...
        | None =>
          ...
          if realtimeRendering {
            let () = nextFrameID := noFrameID
            doRender(16)
          } else {
            let id = Webapi.requestCancellableAnimationFrame(doRender)
            let () = nextFrameID := Some(id)
          }
        }
      let clearPnode = () =>
        while Webapi.Dom.NodeList.length(Webapi.Dom.Node.childNodes(parentNode)) > 0 {
          switch Webapi.Dom.Node.firstChild(parentNode) {
          | None => ()
          | Some(firstChild) =>
            let _removedChild = Webapi.Dom.Node.removeChild(parentNode, ~child=firstChild)
          }
        }
      let oldSub = ref(Tea_sub.none)
      let handleSubscriptionChange = model => {
...
let program: (
  ...
  }: (program<'flags, 'model, 'msg>, Js.null_undefined<Dom.node>, 'flags) => programInterface<'msg>
)
...
let beginnerProgram: (
  ...
  ({model, update, view}, pnode, ()) =>
    standardProgram(
      ...
      pnode,
      (),
    ): (beginnerProgram<'model, 'msg>, Js.null_undefined<Dom.node>, unit) => programInterface<'msg>
)
