@@ocaml.text(" This file is organized roughly in order of popularity. The tags which you'd
...
module Events = {
  ...
  let onInputOpt = (~key="", msg) =>
    onCB("input", key, ev =>
      ...
      | None => None
      | Some(target) =>
        switch Js.Undefined.toOption(target["value"]) {
        | None => None
  ...
  let onCheckOpt = (~key="", msg) =>
    onCB("change", key, ev =>
      ...
      | None => None
      | Some(target) =>
        switch Js.Undefined.toOption(target["checked"]) {
        | None => None
        | Some(value) => msg(value)
        }
      }
