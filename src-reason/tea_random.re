...
let list = (count, Generator(genCmd)) => {
  let rec buildList = (state, i, acc) =>
    if (i > 0) {
      buildList(state, i - 1, [genCmd(state), ...acc]);
    } else {
      acc;
    };
  Generator(state => buildList(state, count, []));
};

let map = (func, Generator(genCmd)) =>
  Generator(state => func(genCmd(state)));
...
let map3 =
    (func, Generator(genCmd1), Generator(genCmd2), Generator(genCmd3)) =>
  Generator(
    state => {
      let res1 = genCmd1(state);
      let res2 = genCmd2(state);
...
let generate = (tagger, Generator(genCmd)) =>
  Tea_cmd.call(callbacks => {
    ...
    Vdom.(callbacks^.enqueue(tagger(genValue)));
  });
